...
class AddPerformanceForm extends ConfirmFormBase
{
	...
	private function addPerformanceDelete($start, $end, $event_nid)	{
		...
		$startdate = new \DateTime(\Drupal::service('date.formatter')->format(strtotime($start), 'custom', 'Y-m-d'));
		$enddate = new \DateTime(\Drupal::service('date.formatter')->format(strtotime($end), 'custom', 'Y-m-d'));
